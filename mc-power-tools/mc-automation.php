<?php
	...
	// loop and print code for all automations on the given day
	if ($total_items != "") {
	$x = 0;
for ($k = 0 ; $k < $total_items; $k++){
	$email_id = $MailChimp->get("automations/" . ($result['automations'][$x]['id']) . "/emails");
	// display all hte required html code in a pretty light blue box
	echo ('<h3 style="font-family: sans-serif;">MailChimp Form Code for Automation: ' . ($result['automations'][$x]['settings']['title']) . '</h3>');
	...
	echo ('&lt;p&gt;Enter your name and email address below then check your email!&lt;/p&gt;');
		echo nl2br ("\n");
	echo ('&lt;form action="' . ($installurl) . '/mc-end-point/lead-magnet.php" method="get"&gt;');
		echo nl2br ("\n");
	echo ('&lt;input type="text" value="" placeholder="Enter your name here..." name="FNAME" class="required" id="mce-FNAME" required&gt;');
		echo nl2br ("\n");
	echo ('&lt;br /&gt;&lt;br /&gt;');
		echo nl2br ("\n");
	echo ('&lt;input type="email" value="" placeholder="Enter your email here..." name="EMAIL" class="required email" id="mce-EMAIL" required&gt;&lt;input type="hidden" name="WORKFLOW_ID" value="' . ($result['automations'][$x]['id']) . '"&gt;&lt;input type="hidden" name="WORKFLOW_EMAIL_ID" value="' . ($email_id['emails'][0]['id']) . '"&gt;');
		echo nl2br ("\n");
